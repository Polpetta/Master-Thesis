...
different kind of characteristics, e.g. a network connectivity in a train has
different networking requirements from the one regarding a company. It is with
these presuppositions that emerges the necessity of service optimization based
on the necessities of the users. To solve this problem in the recent years the
concept of Networks Slice (NS) has been introducted~\cite{alliance20155g}. The
NS aims to logically separate this set of services that present different
use-cases, and to achieve this it expoits the SDN and Network Function Chaining
(NFC) that allows network softwarization~\cite{ordonez2017network}.
...
interest escpecially from the Industries, that is trying to introduce them in
the next mobile network standard, the 5G. More details are provided in
...
a network slice is divided in multiple layers, namely the Service Istance Layer,
the Network Slice Instance Layer and the Resource Layer.
...
contains multiple sub-network instances, tahtn can be virtualized or
non-virtualized.

\paragraph*{Network Functions}
The concept behind NF is to usually elaborate incoming data and forward it to
destination. NF can be physical of virtualized: in the first case is commonly a
set of harware, in the second case instead it can be a software implementation,
decoupled from the hardware where its running. The Virtualized Network Functions
(VNFs), that are recently emerging propose a multitude of challenges, especially
regarding their latency, reliability and efficiency due their virtualized nature
of network function.
...
set o requisites, first of all the possibilty to being created by a template,
defined as Network Slice Template (NST). After the NS and the related resources
are created, a NS should be activable. Obviously, for a NS there should be the
possibility to apport changes on-the-fly, and to decommission or deactivate it,
...
nodes. It is belived tht NFV will trasform the wat the global communications
...
NS aims to provide sufficient packet processing efficency, especially in
environment such as the cloud one.
...
\begin{description}
\item[Virtualised Network Function] that is a software implementation able to fun
  a network function over a NFVI;
\item[NFVI Infrastructure (NFVI)] that includes different physical resuources
  and thei respective virtualization, that have to support the execution of the
  aforementioned VNFs;
...
definition (VNF catalog) and the current NFV instances and the NFVI resources.
...
communicate with the NFV infrstructure, and to control its physical and virtual
...
VNFM can handle one or multiple VNF istances.

\paragraph{NFVO}
We can see how the NFVO is the component with the most connections in the whiole
...
Data repositories can be seen as a sort of databases, holding informations about
the VNF definitions, the VNF instances with the rispective lifecycle status, and
list of the Network Services defined and the resources available and occupied of
the NFVI.

\section{Service Function Caining (SFC)}
...
unit. Another important assumpion is that there is no global or standard SF
...
is defined as \emph{fork} that ends up forming differen branchs. This coulds
leeds to loops, that must be treated consequently.
...
\subparagraph*{Network Serivce Header (NSH)}
The encapsulation has an header that is well defined and descripted in detail
...
lenght of it, an additional metadata field and at the end it has a field to
identify the protocol of the encapsulated data.

The SPH contains, more than the 24-bit Service Path Identifier (SPI), a Service
Index (SI), that provides the location within the SFP.

\subparagraph*{Context header}
There are two kind of context header definition: one with a fixed context header
length (16-Byte) and one with a dynamic context header length.

\noindent Regarding this field, the standard does not make any assuption about
the content, and it is left free to be filled with custom metadata. The only
requirement is to set this filed with zero when is not used.
