...
``virtual'' machines with different configurations, without bringing any
...
component in a hypervisor environment is virtualized, there are possibilities for
the ``guest'' OS (the virtualized one), to access to physical devices (like USB
keys attached on the ``host'' OS).
...
provided a virtualized file-system~\cite{canonico2007virtualization}. In this
...
specific tasks~\cite{fundation2012software} as
...
concept of Networks Slice (NS) has been introduced~\cite{alliance20155g}. The
NS aims to logically separate this set of services that present different
use-cases, and to achieve this it exploits the SDN and Network Function Chaining
...
interest especially from the Industries, that is trying to introduce them in
...
set of hardware, in the second case instead it can be a software implementation,
