...
\item \textbf{VNFs} this are the core components that perform packets
  ...
  particular assumption. Following the RFC 7665, the VNFs are also called
  Service Function (SF)\footnote{From now on the two terminologies will be use
    in a interchangeable way.}. Additional infomation can be found in
  Section~\ref{chap:prjan:sfc}.
...
open-source muti-layer virtual switch. It provides a switching stack for
...
table from software defined as \textit{controllers}. Additionally, it supports the
OpenFlow communication protocol, in order to set previously cited routing
definitions.
