...
Function Virtualization (NFV) design should be composed as follows:\todo{Insert 
image page 11 ETSI vibes specification}
\begin{itemize}
...
from the host operating system, providing better isolation but worste 
...
session recreation\todo{Check if it's the same of Split TCP}.
...
becomes the new packet payload\todo{Make an image showing a packet inside 
another packet, to make easy to understand what we're meaning here}, that can 
...
session table) to slightly improve internal packet transmission. \todo{Insert 
TCP session recreation schema}
